...
if __name__ == '__main__':
    ...
    # Adding a dummy call to compile the functions
    euler_integration(y0, t0, dt, t0+4*dt)
    ...
    '''
    # Creating the analytical solution at all times for comparison... for some reason
    anEvals = int((tmax-t0)/dt)
    ya = np.zeros(int((tmax-t0)/dt))

    for i in range(anEvals):
        ya[i] = -1*np.cos(t0+i*dt)
    '''

    print("Solution for time t={0:.4f}: y(t)={1:.8f} ".format( t[-1],y[-1]))
    #print("Analytical Solution for time t={0:.4f}: y(t)={1:.8f} ".format( t[-1],ya[-1]))
